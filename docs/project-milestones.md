...
| M5 資源信箱讀取確認 | 確認流程帳號可讀取三台公務車資源行事曆 | 受阻擋 | 2026-07-02 已實測，待 Exchange Calendar 權限 |
...
目前位於 M5。已建立測試流程並確認 Outlook 連接器驗證正常，但三台資源行事曆未出現在 Calendar ID 選單；直接以資源信箱 Email 測試會回傳「ID 格式不正確」。完成 Exchange Calendar `Reviewer` 權限與共享行事曆可見性設定後，需重新測試三台車，通過才進入 M6。
