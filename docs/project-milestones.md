...
| M5 資源信箱讀取確認 | 取得三台 Calendar ID 並讀取未來 7 天事件 | 進行中／受阻 | 2026-07-18 候選 GUID V3 實測回傳 `ErrorInvalidIdMalformed` |
...
目前位於 M5。已確認本專案不需要 Exchange Administrator。資源信箱 Email 與候選 GUID `6049e1d1-b34c-4cca-b530-2c7c4b77abe9` 均已被 V3 連接器判定為無效 ID。2026-07-18 的真正 GUID 輸入實測回傳 `ErrorInvalidIdMalformed`，下一步需取得連接器可接受的 Calendar ID 或採用 O365 E1 標準替代方案。
