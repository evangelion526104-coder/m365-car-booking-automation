...
| M5 資源信箱讀取確認 | 確認流程帳號可讀取三台公務車資源行事曆 | 受阻擋 | 2026-07-02 已確認流程帳號無 Exchange 管理角色，待管理員設定 |
...
目前位於 M5。已建立測試流程並確認 Outlook 連接器驗證正常，但三台資源行事曆未出現在 Calendar ID 選單；直接以資源信箱 Email 測試會回傳「ID 格式不正確」。另已確認流程帳號沒有 Exchange 收件者管理功能，需由 Exchange 管理員登入或代為設定 Calendar `Reviewer` 權限。設定後需重新測試三台車，通過才進入 M6。
