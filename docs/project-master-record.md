...
| 目前開發階段 | M5 已確認流程帳號沒有 Exchange 收件者管理權限，待管理員登入或代為設定 Calendar 權限 |
| 專案完成度 | 69% |
...
- 需由 Exchange 管理員授予流程帳號三台 Calendar 資料夾 `Reviewer` 權限，再逐台重測。
- 已驗證 `ad.general@alp.global` 進入 Exchange 管理中心後只有帳戶自助功能，無法管理收件者或信箱。
...
- Exchange 管理角色存取測試已完成，確認流程帳號沒有修改三台資源信箱權限的管理功能。
...
| Outlook | 受阻擋 | 42% | 標準連線可用；流程帳號沒有 Exchange 收件者管理權限，待管理員設定 Calendar 權限 |
...
| 優先順序 | 工作項目 | 說明 |
|---:|---|---|
| 1 | 完成 Exchange Calendar 權限 | 授予 `ad.general@alp.global` 三台資源行事曆 `Reviewer` 權限，確認 Calendar ID 選單可見 |
...
目前唯一 P0 阻擋是：三台資源行事曆未出現在 Power Automate Calendar ID 選單，資源信箱 Email 也不能直接作為 Calendar ID；且 `ad.general@alp.global` 沒有 Exchange 收件者管理權限。具備 Exchange 管理角色的帳號完成 Calendar `Reviewer` 權限後，需逐台通過讀取測試，才可開始建立正式同步與 Teams 通知流程。
