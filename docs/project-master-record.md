...
| 目前開發階段 | M5 資源行事曆讀取已實測，待 Exchange 管理員補齊 Calendar 權限與可用 Calendar ID |
| 專案完成度 | 69% |
...
| Outlook | 受阻擋 | 42% | 標準連線可用；資源行事曆缺少可用 Calendar ID，待 Exchange 權限 |
...
目前唯一 P0 阻擋是：三台資源行事曆未出現在 Power Automate Calendar ID 選單，資源信箱 Email 也不能直接作為 Calendar ID。Exchange 管理員完成 Calendar `Reviewer` 權限後，需逐台通過讀取測試，才可開始建立正式同步與 Teams 通知流程。
