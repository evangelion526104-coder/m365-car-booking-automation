...
| FT-103 | Outlook 取消預約 | SharePoint `事件同步狀態` 更新為 `已取消`、`已取消 = 是`，並停止後續通知 | **未通過（發現新問題）**：刪除 Outlook 事件後，流程的「取得多個項目」不再回傳該事件，目前無對應機制偵測事件消失並將 SharePoint 紀錄標記為已取消；需於下一階段補強比對邏輯（例如比對 SharePoint 既有紀錄與本次讀取結果，找出消失的 Event ID 並標記取消）。 |
...
| SG-T04 | 行事曆取消測試 | 取消 Outlook 預約 | SharePoint 更新為 `已取消`，後續 Teams 通知停止 | **未通過（發現新問題，同 FT-103）**：目前流程無法偵測 Outlook 事件被刪除／取消，SharePoint 紀錄不會自動標記為已取消。 |
| SG-T05 | 行事曆異動測試 | 修改時間、車輛、主旨或借用人 | SharePoint 同步更新，重算 `預計通知時間`，舊卡片失效 | 部分通過（時間異動已測試通過；車輛、主旨、借用人異動與舊卡片失效機制尚未測試，待 Teams 卡片流程開發後一併驗證） |
| SG-T06 | 舊卡片送出測試 | 使用者送出已取消或已異動案件的舊 Teams 卡片 | 不更新舊資料，提示「此借用已取消或已變更，請重新確認最新借用資訊。」 | 待測（需待 Teams Adaptive Card 流程開發後測試） |
