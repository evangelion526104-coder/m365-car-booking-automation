...
### 正式流程：公務車行事曆同步至SharePoint（v0.2.14 已修正「更新項目」時間欄位 +8 小時位移錯誤）
...
| 執行期驗證 | v0.2.12 開啟排程後所有執行紀錄皆失敗；v0.2.13 修正四種執行期錯誤後運行紀錄首次出現「測試成功」；v0.2.14 進一步發現並修正「更新項目」時間欄位比「建立項目」多 8 小時的嚴重錯誤，已以 SharePoint REST API 驗證恢復正確 |
| 尚待驗證 | 正式紀錄（Id=1、Id=2）於下次排程後時間欄位是否已回復正確；P3 其餘情境測試（取消、時間異動、整天借用、重複觸發等） |

詳見 `release-notes/v0.2.14.md`（前次見 `release-notes/v0.2.13.md`）。
...
| 「更新項目」時間欄位 +8 小時位移錯誤修正 | 已完成 | v0.2.14 修正並以 SharePoint REST API 驗證通過；正式紀錄回復情形待下次排程後抽查 |
...
## v0.2.14 更新 - 修正「更新項目」時間欄位 +8 小時位移嚴重錯誤

2026-07-24：執行 P3 情境測試時，以測試紀錄（SharePoint Item Id=4）連續三次 SharePoint REST API 比對，發現「更新項目」（Update，PATCH/MERGE）動作寫入借用起始時間、借用結束時間、預計通知時間、事件最後修改時間時，比「建立項目」（Create，POST）動作多了 8 小時（恰為 Taipei 時區的 UTC 位移量），即使兩個動作對這些欄位使用的運算式完全相同：

1. 建立當下（Create 執行）：借用起始時間正確為「2026/7/25 上午 03:00」。
2. 第一次排程自動觸發「更新項目」後：同一欄位變成「上午 11:00」（+8 小時）。
3. 第二次排程自動觸發「更新項目」後：仍維持「上午 11:00」，未再疊加，證實是固定位移而非累積誤差。

以「剖析 JSON」動作的原始輸出比對前後兩次執行，確認 Graph 回傳的來源資料完全相同，排除來源資料變動的可能性。鎖定成因為：SharePoint 連接器的 Create（POST）與 Update（PATCH/MERGE）動作，對「不含時區後綴的模糊時間字串」（例如 `2026-07-25T03:00:00`，無 `Z`、無 UTC 位移量）採用了不同的解讀邏輯——Create 會將其視為目的地時區的本地時間並正確反推為 UTC 儲存；Update 則直接視為 UTC 儲存，導致顯示時多加了一次 +8 小時。

修正範圍限定在「更新項目」動作（「建立項目」維持不變，因其運作正確）：

- 借用起始/結束時間、最後同步時間、事件最後修改時間：移除 `convertTimeZone` 轉換，改為直接輸出 UTC 原始時間並明確加上 `Z` 時區後綴（如 `formatDateTime(items('套用至各項_1')?['start']?['dateTime'],'yyyy-MM-ddTHH:mm:ssZ')`）。
- 預計通知時間：因參照與「建立項目」共用的「編輯 1」Compose 輸出（`outputs('編輯_1')`），不直接修改 Compose 本身，改為在「更新項目」引用處以 `addHours(outputs('編輯_1'),-8)` 反推回 UTC 再補 `Z` 後綴。

修正後儲存流程確認零錯誤，手動觸發測試（測試紀錄已存在，走「更新項目」分支），以 SharePoint REST API 驗證借用起始/結束時間、預計通知時間皆恢復與 Create 當下一致的正確值。

本錯誤直接影響 v0.2.13「判定：通過」結論的完整性——該結論僅驗證了 Create 路徑，未涵蓋排程自動觸發的 Update 路徑；正式（非測試）紀錄 Id=1、Id=2 過去因走過 Update 路徑，可能已被寫入錯誤時間，待其下次排程自動更新後應會被本次修正覆寫回正確值，建議另行抽查確認。

詳細版本紀錄見 `release-notes/v0.2.14.md`。

下一步：抽查正式紀錄 Id=1、Id=2 於下次排程後時間欄位是否已回復正確，繼續執行 P3 其餘情境測試（取消、時間異動、整天借用、重複觸發等），通過後將 `是否測試資料` 切換為正式運作邏輯、建立 Concurrency Control、刪除暫存測試流程。
